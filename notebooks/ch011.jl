...
[var(df.x), var(df.y), cov(x, y), cor(x, y), var(df.x + df.y), cov(x, z), cor(x, z)]
...
let
	...
	tdf = df[df.x .< -2, :]
	scatter!(tdf.x, tdf.y; 
		marker=:cross, markersize=1, color=:red, lab="x < -2")
	tdf = df[df.x .>= -2 .&& df.x .< 2, :]
	scatter!(tdf.x, tdf.y; 
		marker=:cross, markersize=1, color=:yellow, lab="-2 < x < 2")
	tdf = df[df.x .>= 2, :]
	scatter!(tdf.x, tdf.y; 
